...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    """
    # Note: ANTHROPIC_API_KEY no longer required after Agent SDK migration
    # Agent SDK uses 'claude auth login' authentication
    required_vars = [
        "CLAUDE_CODE_PATH",
    ]
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    """
    safe_env_vars = {
        # Anthropic Configuration (no longer required after Agent SDK migration)
        # Agent SDK uses 'claude auth login' instead of API key
