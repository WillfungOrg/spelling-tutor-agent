...
This module provides both subprocess-based (legacy) and Agent SDK-based (new)
implementations. The SDK-based approach is now the default, with subprocess
...
# Import Agent SDK async functions
from .agent_sdk import (
    prompt_claude_code_async,
    prompt_claude_code_with_retry_async,
...
def prompt_claude_code_with_retry(
    ...
    This is a sync wrapper around the async Agent SDK implementation.
    For async code, use prompt_claude_code_with_retry_async() directly.
    ...
    """
    return asyncio.run(
        prompt_claude_code_with_retry_async(request, max_retries, retry_delays)
    )


def _prompt_claude_code_with_retry_subprocess(
    request: AgentPromptRequest,
    max_retries: int = 3,
    retry_delays: List[int] = None,
) -> AgentPromptResponse:
    """[DEPRECATED] Execute with retry using subprocess (legacy implementation).

    This function is kept for emergency fallback only.
    Use prompt_claude_code_with_retry() which now uses Agent SDK by default.
    """
    ...
    for attempt in range(max_retries + 1):  # +1 for initial attempt
        ...
        response = _prompt_claude_code_subprocess(request)
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    Uses subprocess-based execution (synchronous).
    For async code with Agent SDK, use prompt_claude_code_async() directly.
    """
    return _prompt_claude_code_with_retry(request)


def _prompt_claude_code_subprocess(request: AgentPromptRequest) -> AgentPromptResponse:
    """[DEPRECATED] Execute Claude Code with subprocess (legacy implementation).

    This function is kept for emergency fallback only.
    Use prompt_claude_code() which now uses Agent SDK by default.
    """
...
def _execute_template_with_retry(
    request: AgentPromptRequest, max_retries: int = 3
) -> AgentPromptResponse:
    """Execute template with subprocess and automatic retry logic.
    ...
    """
    retry_delays = [2, 5, 10]  # Delays in seconds between retries
    last_response = None
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    Uses subprocess-based execution (synchronous).
    For async code with Agent SDK, use execute_template_async() directly.
    ...
    if request.working_dir:
        ...
        # __file__ is in adw/adw_modules/agent.py
        # So we go up 2 levels to get to project root
        adw_modules_dir = os.path.dirname(os.path.abspath(__file__))
        adw_dir = os.path.dirname(adw_modules_dir)
        working_dir = os.path.dirname(adw_dir)

    # Load and expand the slash command template
    from .agent_sdk import load_and_expand_slash_command

    try:
        expanded_prompt = load_and_expand_slash_command(
    ...
    # Note: AgentTemplateRequest doesn't have timeout_seconds, use default
    ...
    # Execute using subprocess-based implementation
    return _execute_template_with_retry(prompt_request)
