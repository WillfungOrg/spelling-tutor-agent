...
def get_default_branch() -> str:
    ...
    Returns:
        The default branch name (e.g., 'origin/main' or 'origin/master')
    """
...
def create_worktree(adw_id: str, branch_name: str, logger: logging.Logger, working_dir: Optional[str] = None) -> Tuple[str, Optional[str]]:
    ...
    Args:
        ...
        working_dir: Optional working directory (for multi-repo webhook). If None, uses library root.
    ...
    # Get project root
    if working_dir:
        # Multi-repo mode: use provided working directory (target repo)
        project_root = working_dir
    else:
        # Single-repo mode: use library root (parent of adw directory)
    ...
    # First, fetch latest changes from origin with explicit ref updates
    ...
    # Update symbolic ref for origin/HEAD to ensure default branch detection works
    subprocess.run(
    ...
    # Create the worktree using git, branching from the default branch
    # Use -b to create the branch as part of worktree creation
    default_branch = get_default_branch()

    # Verify the default branch exists before trying to create worktree
    ...
    if verify_result.returncode != 0:
        error_msg = f"Default branch {default_branch} not found after fetch. stderr: {verify_result.stderr}"
        logger.error(error_msg)
        return None, error_msg

    logger.info(f"Creating worktree from {default_branch}")
    cmd = ["git", "worktree", "add", "-b", branch_name, worktree_path, default_branch]
