...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    """
    # ANTHROPIC_API_KEY is optional when using OAuth
    # Only CLAUDE_CODE_PATH is required
    required_vars = [
        "CLAUDE_CODE_PATH",
    ]
    ...
    # Warn if ANTHROPIC_API_KEY is missing (using OAuth)
    if not os.getenv("ANTHROPIC_API_KEY"):
        msg = "ℹ️  ANTHROPIC_API_KEY not set, using Claude Code OAuth authentication"
        if logger:
            logger.info(msg)
        else:
...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    """
    safe_env_vars = {
        # Anthropic Configuration (choose one: OAuth token or API key)
