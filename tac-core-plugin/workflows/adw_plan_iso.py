...
from adw_modules.learning_integration import trigger_learning_system
...
def main():
    """Main entry point."""
    # Track workflow start time for learning system
    ...
    if not valid:
        ...
        # Copy tac-core-plugin into worktree if it exists in parent repo
        parent_plugin = os.path.join(os.getcwd(), "tac-core-plugin")
        worktree_plugin = os.path.join(worktree_path, "tac-core-plugin")
        if os.path.exists(parent_plugin) and not os.path.exists(worktree_plugin):
            logger.info(f"Copying tac-core-plugin from parent repo to worktree")
            import shutil
            # Exclude directories that should never be copied to worktrees
            # to prevent recursive directory structures
            def ignore_dirs(dir, files):
                return {'trees', 'agents', 'repos', '.tac', '__pycache__', '.pytest_cache', '.git'}
            shutil.copytree(parent_plugin, worktree_plugin, ignore=ignore_dirs)
    ...
    # Extract the file path - look for specs/issue-{number}-adw-{id}...md pattern
    # Claude might include explanation text, so we need to extract just the path
    import re
    import glob
    path_pattern = r'(specs/issue-\d+-adw-[\w\-]+\.md)'
    ...
    if match:
        ...
        # Fallback 1: try to use the last line (in case Claude just outputs the path)
        lines = response_text.strip().split('\n')
        potential_path = lines[-1].strip()
        if potential_path.startswith('specs/') and potential_path.endswith('.md'):
            plan_file_path = potential_path
            logger.info(f"Using last line as plan file path: {plan_file_path}")
        else:
            # Fallback 2: search for created spec file directly in worktree
            # Pattern: specs/issue-{number}-adw-{adw_id}-*.md
            search_pattern = f"specs/issue-{issue_number}-adw-{adw_id}-*.md"
            spec_files = glob.glob(os.path.join(worktree_path, search_pattern))

            if spec_files:
                # Get the most recently created file
                latest_spec = max(spec_files, key=os.path.getctime)
                plan_file_path = os.path.relpath(latest_spec, worktree_path)
                logger.info(f"Found created spec file directly: {plan_file_path}")
            else:
