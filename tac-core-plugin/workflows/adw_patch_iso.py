...
def trigger_learning_system(adw_id: str, issue_number: str, workflow_name: str, start_time: datetime,
        workflow_type="patch"
    ):
...
def main():
    ...
    if not branch_name:
        ...
        if existing_branch:
            ...
            # Patch workflows default to /chore (maintenance tasks)
            # No need to classify - patches are typically bug fixes or maintenance
            logger.info("Patch workflow: defaulting to /chore")
            issue_command = "/chore"
    ...
    if worktree_path and os.path.exists(worktree_path):
        ...
        if os.path.exists(parent_plugin) and not os.path.exists(worktree_plugin):
            ...
            import shutil
            shutil.copytree(parent_plugin, worktree_plugin)
