...
# Re-export async functions from agent_sdk for backward compatibility
from .agent_sdk import execute_template_async
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    # This loads the template file from .claude/commands/ and replaces $1, $2, etc. with args
    from .agent_sdk import load_and_expand_slash_command

    # Ensure working_dir is set (required for template loading)
    ...
    try:
        ...
        # If template not found, log error and use fallback
        print(f"Warning: Template not found for {request.slash_command}: {e}")
        print(f"Falling back to command invocation format")
        command_name = request.slash_command
        if command_name.startswith("/"):
            command_name = command_name[1:]
        prompt = f"tac-core:{command_name} {' '.join(request.args)}"
