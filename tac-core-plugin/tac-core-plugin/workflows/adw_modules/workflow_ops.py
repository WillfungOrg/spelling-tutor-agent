...
from adw_modules.worktree_ops import get_default_branch
...
AGENT_STRATEGIST = "meta_strategist"  # Meta-ADW strategy advisor

# Available ADW workflows for runtime validation
AVAILABLE_ADW_WORKFLOWS = [
    # Meta-ADW workflows
    "meta_adw_strategy",  # Meta-ADW intelligent strategy selection
...
async def extract_adw_info_async(text: str, temp_adw_id: str) -> ADWExtractionResult:
    """Async version of extract_adw_info for use in async contexts like webhooks.
    Extract ADW workflow, ID, and model_set from text using classify_adw agent.
    Returns ADWExtractionResult with workflow_command, adw_id, and model_set."""
    ...
    try:
        response = execute_template(request)  # Use sync version (execute_template is not async)
...
def generate_branch_name(
    ...
    # Extract branch name from output (AI might include explanations)
    # Look for the branch name pattern: type-issue-NUM-adw-ID-description
    import re
    output = response.output.strip()

    # Try multiple patterns to handle different AI output formats
    patterns = [
        # Format 1: type-issue-NUM-adw-ID-description (full format)
        r'([a-z]+)-issue-(\d+)-adw-([a-f0-9]{8})-([a-z0-9-]+)',
        # Format 2: type-NUM-ID-description (simplified format)
        r'([a-z]+)-(\d+)-([a-f0-9]{8})-([a-z0-9-]+)',
    ]
    ...
    for pattern in patterns:
        ...
        if matches:
            ...
            match = matches[-1]
            # Normalize to full format with "issue" and "adw" keywords
            branch_name = f"{match[0]}-issue-{match[1]}-adw-{match[2]}-{match[3]}"
            logger.info(f"Extracted branch name from output using pattern: {pattern}")
            break

    if not branch_name:
        # Fallback: try to use the entire output if it looks like a branch name
        # (single line, no whitespace except hyphens)
        if '\n' not in output and ' ' not in output and output.count('-') >= 4:
            branch_name = output
            logger.info(f"Using entire output as branch name: {branch_name}")
        else:
            error_msg = f"Could not extract valid branch name from AI output: {output[:200]}"
            logger.error(error_msg)
            return None, error_msg

    # Validate branch name is not empty
    if not branch_name:
        error_msg = "Generated branch name is empty"
        logger.error(error_msg)
        return None, error_msg
