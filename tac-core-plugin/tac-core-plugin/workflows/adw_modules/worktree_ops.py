...
def create_worktree(adw_id: str, branch_name: str, logger: logging.Logger) -> Tuple[str, Optional[str]]:
    ...
    """
    # Get project root using git (finds the actual repository root)
    result = subprocess.run(
    ...
    if result.returncode != 0:
        error_msg = f"Failed to find git repository root: {result.stderr}"
    ...
    # Create the worktree using git, branching from default branch
    ...
    # Clean up .tac directory from worktree (should use root's .tac instead)
    # Learning data is project-level, not worktree-level
    worktree_tac = os.path.join(worktree_path, ".tac")
    if os.path.exists(worktree_tac):
        try:
            shutil.rmtree(worktree_tac)
            logger.info(f"Removed .tac from worktree (using root .tac for learning data)")
        except Exception as e:
            logger.warning(f"Could not remove .tac from worktree: {e}")
            # Non-fatal, continue
...
def get_worktree_path(adw_id: str) -> str:
    ...
    if result.returncode != 0:
        # Fallback to path navigation if git command fails
